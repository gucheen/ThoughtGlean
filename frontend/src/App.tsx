...
import { createNote, db, deleteAttachment, deleteLibraryMetadata, exportBackup, markdownExport, migrateLegacyLibrary, newID, now, queue, queueLibrarySnapshot, readLibraryMetadata, restoreBackup, saveAttachment, saveNote, updateAttachmentAlt, writeLibraryMetadata, type Attachment, type Note, type NoteSource } from "./db";
...
export function App() {
  ...
  return <div className={`app-shell ${selected && editing ? "is-detail-editing" : ""}`}>
    ...
    }} type="search" placeholder="搜索标题、正文、来源和图片名" /><kbd>↑↓</kbd><kbd>↵</kbd></label><button className="button button-primary top-new" onClick={() => { showView(view === "trash" ? "recent" : view); requestAnimationFrame(() => captureInput.current?.focus()); }}><span>＋</span><span className="button-label">新记录</span></button></div></header>
...
function DateGroup({ dateKey, notes, noteByID, sourceByNote, continuationCounts, query, searchHints, keyboardSelectedID, view, open, update, restore }: { dateKey: string; notes: Note[]; noteByID: Map<string, Note>; sourceByNote: Map<string, NoteSource>; continuationCounts: Map<string, number>; query: string; searchHints: Map<string, string>; keyboardSelectedID?: string; view: View; open: (noteID: string) => void; update: (note: Note, patch: Partial<Note>) => Promise<void>; restore: (note: Note) => Promise<void> }) {
  ...
  return <section className="date-group"><h2 className="date-heading">{heading.label}<small>{heading.detail}</small></h2><div className="date-notes">{notes.map(note => <article className={`note-row ${keyboardSelectedID === note.id ? "keyboard-selected" : ""}`} data-note-id={note.id} key={note.id}>
    <button className="note-row-open" onClick={() => open(note.id)}><time className="note-time">{new Date(note.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</time><span className="note-row-main"><strong className="note-row-title"><HighlightedText text={note.title} query={query} /></strong><span className="note-excerpt"><HighlightedText text={excerptForSearch(note.content, query)} query={query} /></span>{(note.continuedFromId || continuationCounts.has(note.id) || searchHints.has(note.id)) && <span className="note-meta">{note.continuedFromId && <span className="relation-label">{isConflict(note) ? "同步冲突" : `续写自：${noteByID.get(note.continuedFromId)?.title ?? "原记录暂不可用"}`}</span>}{continuationCounts.has(note.id) && <span className="relation-label">{continuationCounts.get(note.id)} 条后续续写</span>}{searchHints.get(note.id) && <span><HighlightedText text={searchHints.get(note.id)!} query={query} /></span>}</span>}</span><span className="note-row-facts"><span>更新 {new Date(note.updatedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>{sourceByNote.get(note.id) && <span className="note-row-source" title={sourceByNote.get(note.id)!.url}>来源 · {sourceByNote.get(note.id)!.title || sourceByNote.get(note.id)!.url}</span>}</span></button>
...
function NoteContext({ note, notes, onSelect, onContinue }: { note: Note; notes: Note[]; onSelect: (noteID: string) => void; onContinue: () => void }) {
  ...
  const timeline = notes.filter(item => !item.deletedAt || item.id === note.id).sort((a, b) => a.createdAt.localeCompare(b.createdAt) || a.id.localeCompare(b.id));
  ...
  return <aside className="context-rail">
    <section className="context-section" aria-label="续写关系"><h2>续写关系</h2><p className="context-copy">从哪条记录接着写，又有哪些后续想法。</p>
